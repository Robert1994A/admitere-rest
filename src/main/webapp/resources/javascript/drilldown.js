...
(function($) {
	...
	$.fn.dcDrilldown = function(options) {
		...
		return this
				.each(function(options) {
										...
										if ($('> ul', $activeLi).length) {
											if ($($link).hasClass(
												...
												$('ul a', $activeLi)
														.removeClass(
																defaults.classActive);
												resetDrilldown($dcDrilldownObj,
														$dcWrapper);
											} else {
												actionDrillDown($activeLi,
														$dcWrapper,
														$dcDrilldownObj);
											}
										}

										// Prevent browsing to link if has child
										// links
					...
					function setUpDrilldown() {
						...
						$arrow = '<span class="' + defaults.classIcon
								+ '"></span>';
						...
						totalWidth += 'px';
						var itemHeight = $('li', $dcDrilldownObj).outerHeight(
								true);
						...
						$('li', $dcDrilldownObj)
								.each(
										function() {
											...
											if ($('> ul', this).length) {
												$(this).addClass(
														defaults.classParent);
												$('> a', this)
														.addClass(
																defaults.classParentLink)
														.append($arrow);
						...
						$('ul', $dcWrapper).find('li.active').parents('li')
								.addClass('active');
					}
					...
					$('#' + idHeader).on(
							...
							function(e) {
								...
								if ($(this).hasClass('link-back')) {
									...
									$('a.' + defaults.classActive + ':last',
											$dcDrilldownObj).removeClass(
											defaults.classActive);
								} else {
									// Get link index
									var linkIndex = parseInt($(this).index(
											'#' + idHeader + ' a'));
									if (linkIndex == 0) {
										$('a', $dcDrilldownObj).removeClass(
												defaults.classActive);
									} else {
										// Select equivalent active link
										linkIndex = linkIndex - 1;
										$(
												'a.' + defaults.classActive
														+ ':gt(' + linkIndex
														+ ')', $dcDrilldownObj)
												.removeClass(
														defaults.classActive);
		...
		function actionDrillDown(element, wrapper, obj) {
			...
			// var getParentText =
			// $(wrapper).find('.'+defaults.classMenu).data('category');
				...
				if (getNewBreadcrumb == defaults.defaultText) {
					$('ul', $header).append(
							'<li><a href="#" class="first"> '
									+ defaults.resetText + '</a></li>');
				} else {
					$('ul', $header).append(
							'<li><a href="#"> ' + getNewBreadcrumb
									+ '</a></li>');
				}
			}
			...
			if (defaults.linkType == 'link') {
				if (!$('a', $header).length) {
					$($header).prepend(
							'<ul><li><a href="#" class="first"> '
									+ defaults.resetText + '</a></li></ul>');
				}
			}
			// Update header text
		...
		function actionDrillUp(element, obj, wrapper) {
			...
			var activeIndex = $(activeLink).index('.' + defaults.classActive,
					wrapper);
		...
		function resetDrilldown(obj, wrapper) {
			...
			if (defaults.linkType == "link") {
				if ($('a.' + defaults.classActive + ':last', obj).parent('li').length) {
					var lastActive = $('a.' + defaults.classActive + ':last',
							obj).parent('li');
